...
class SharepointFilesIndexer:
    def __init__(self):
        ...
        self.drive_name = os.getenv("SHAREPOINT_DRIVE_NAME")
    ...
    #             drive_name=self.drive_name,              # <-- IMPORTANT
    ...
    async def run(self) -> None:
        ...
        if not self.connector_enabled:
            logging.info("[sharepoint_files_indexer] SharePoint connector is disabled. "
        ...
        try:
            files = self.sharepoint_data_reader.retrieve_sharepoint_files_content(
                site_domain=self.site_domain,
                site_name=self.site_name,
                drive_name=self.drive_name,       # ✅ drives are handled in reader
                folder_path=self.folder_path,
                file_formats=self.file_formats
            )
            ...
            logging.info(f"[sharepoint_files_indexer] Retrieved {number_files} files from SharePoint "
                         f"(drive: {self.drive_name}, folder: {self.folder_path}).")

        except Exception as e:
            logging.error(f"[sharepoint_files_indexer] Failed to retrieve files. "
                          f"Check your sharepoint configuration environment variables. Error: {e}")
            return
