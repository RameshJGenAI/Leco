...
class SharePointDataReader:
    ...
    def retrieve_sharepoint_files_content(
        ...
        site_domain: str,
        site_name: str,
        drive_name: Optional[str] = None,   # already present
        ...
        # Ensure we have a token if caller forgot to authenticate
        ...
        logging.info(f"[sharepoint_files_reader] Using drive: {drive_name or 'default'} ({drive_id})")

        files = self._get_files(site_id, drive_id, folder_path, minutes_ago, file_formats)
        if not files:
            logging.info("[sharepoint_files_reader] No files found in the site's drive")
            return None

        return self._process_files(site_id, drive_id, folder_path, file_names, files, file_formats)
    ...
    def _get_drive_id(
        ...
        """
        Get the drive ID for a specific document library (drive) in a Microsoft Graph site.
        If drive_name is not provided, return the default drive (usually 'Documents').
        """
            ...
            if drive_name:
                for drive in drives:
                    if drive.get("name", "").lower() == drive_name.lower():
                        logging.debug(
                            f"[sharepoint_files_reader] Found drive '{drive_name}' with ID: {drive['id']}"
                        )
                        return drive["id"]

                logging.error(
                    f"[sharepoint_files_reader] Drive named '{drive_name}' not found in site {site_id}"
                )
                return None
            else:
                default_drive_id = drives[0]["id"] if drives else None
                logging.debug(
                    f"[sharepoint_files_reader] Returning default drive ID: {default_drive_id}"
                )
    ...
    def _get_files_in_site(
        ...
        """
        Get a list of files in a site's drive with pagination support.
        """
        access_token = access_token or self.access_token
        ...
        while url:
            try:
                logging.info(f"[sharepoint_files_reader] Fetching files from: {url}")
                response = self._make_ms_graph_request(url, access_token)
                batch = response.get("value", [])

                for file in batch:
                    if "file" not in file:  # skip folders
                        continue
                    ...
                    if time_limit:
                        ...
                        if created < time_limit and modified < time_limit:
                            continue

                    # Format filter
                    if file_formats and not any(file["name"].lower().endswith(f".{fmt.lower()}") for fmt in file_formats):
                        continue
                ...
                logging.error(f"[sharepoint_files_reader] Error in _get_files_in_site: {err}")
                break

        logging.info(f"[sharepoint_files_reader] Retrieved {len(files)} files from drive {drive_id}")
        return files
    ...
    def _get_file_permissions(
        ...
        """
        Get the permissions of a file in a specific drive.
        """
        if access_token is None:
            access_token = self.access_token
    ...
    def _get_file_content_bytes(
        ...
        try:
            ...
            if response.status_code != 200:
                logging.error(
                    f"[sharepoint_files_reader] Failed to retrieve file content. "
    ...
    def _get_site_and_drive_ids(
        ...
        drive_id = self._get_drive_id(site_id, drive_name)
        if not drive_id:
            logging.error("[sharepoint_files_reader] Failed to retrieve drive ID")
            return None, None
